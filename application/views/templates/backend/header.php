		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$( table ).delegate( '#tr_clone_del', 'click', function () {
				    ...
				    //$( thisRow ).clone().removeAfter( thisRow ).find( 'input:text' ).val( '' );
				});

				$( tables ).delegate( '#tr_clone_adds', 'click', function () {
				    var thisRow = $( this ).closest( 'tr' )[0];
				    //$("#tr_clone_dels").html("alt","0");
				    $( thisRow ).find( '#tr_clone_dels' ).attr( 'alt', '0' );
				    $( thisRow ).clone().insertAfter( thisRow ).find( 'input:text,input:hidden' ).val( '' );
				});
				$( tables ).delegate( '#tr_clone_dels', 'click', function () {
					var data = $(this).attr('alt');
					if(data!='0') {
						if (confirm('Anda yakin untuk menghapus data ini?')) {
							$.ajax({
		                        url: '<?php echo base_url();?>produk/produk_detail_delete/'+data,
		                        cache: true,
