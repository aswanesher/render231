...
class Biaya_resi extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('biaya_resi_model');
    ...
    public function index()
    {
        ...
        $mod=$this->permission_model->get_data_module('biaya_resi');
        ...
        if($this->session->userdata('logged_in')) {
            if(!empty($per->is_view)&&($per->is_view=='true')) {
                ...
                $data['jumlah']= $this->biaya_resi_model->jumlah();

                // Config page
                $config['base_url'] = base_url().'/biaya_resi/index';
                $config['total_rows'] = $data['jumlah'];
                $config['per_page'] = 20;

                $dari = $this->uri->segment('3');
                $data['biaya_resi']=$this->biaya_resi_model->get_dataall($param,$config['per_page'],$dari);
                ...
                $data['judul_panel']='Pelaporan Biaya Resi';
                $data['tambah']=$per->is_add;
                $data['edit']=$per->is_edit;
                $data['hapus']=$per->is_delete;

                $view = 'templates/backend/biaya_resi_modul/biaya_resi_data';
                show($view, $data);
    ...
    public function get_data()
    {
        ...
        $mod=$this->permission_model->get_data_module('biaya_resi');
        $per=$this->permission_model->get_data_akses($mod->module_id,$usr['group']);
        $list = $this->biaya_resi_model->get_datatables();
        $data = array();
        $no = '1';
        foreach ($list as $person) {
            ...
            $row[] = 'Rp. '.number_format((float)$ongkir, '0', '.', ',');
            $row[] = 'Rp. '.number_format((float)$transfer, '0', '.', ',');
            if($person->selisih<'0') {
                $row[] = '<span class="label label-lg label-danger">'.'Rp. '.number_format((float)$selisih, '0', '.', ',').'</span>';
            } else {
                $row[] = '<span class="label label-lg label-success">'.'Rp. '.number_format((float)$selisih, '0', '.', ',').'</span>';
            } 
            $row[] = $person->tanggal;
            //add html for action
            if($per->is_edit=='true') { 
                $edit = '<a class="green" href="'.base_url().'biaya_resi/biaya_resi_edit/'.$person->id_resi.'"><i class="ace-icon fa fa-pencil bigger-130"></i></a>';
            } else {
                $edit = '';
            }
            if($per->is_delete=='true') { 
                $hapus = '<a class="red" href="'.base_url().'biaya_resi/biaya_resi_delete/'.$person->id_resi.'"><i class="ace-icon fa fa-trash-o bigger-130"></i></a>';
            } else {
                $hapus = '';
            }
            $row[] = $edit." ".$hapus;
            ...
            if($per->is_edit=='true') { 
                $edits = '<li>
                                    <a href="'.base_url().'biaya_resi/biaya_resi_edit/'.$person->id_resi.'" class="tooltip-success" data-rel="tooltip" title="Edit">
                                        <span class="green">
            ...
            if($per->is_delete=='true') { 
                $hapuss= '<li>
                                    <a href="'.base_url().'biaya_resi/biaya_resi_delete/'.$person->id_resi.'" class="tooltip-error" data-rel="tooltip" title="Delete">
                                        <span class="red">
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->biaya_resi_model->jumlah(),
                        "recordsFiltered" => $this->biaya_resi_model->count_filtered(),
                        "data" => $data,
                );
    ...
    public function biaya_resi_add()
    {
        $usr=$this->session->userdata('logged_in');
        $mod=$this->permission_model->get_data_module('biaya_resi');
        $per=$this->permission_model->get_data_akses($mod->module_id,$usr['group']);
        ...
        if($this->session->userdata('logged_in')) {
            if(!empty($per->is_add)&&($per->is_add=='true')) {
                ...
                $data['judul']=$datas->website_title;
                $data['company']=$datas->company_name;
                $data['judul_panel']='Tambah Biaya Resi';

                $data['ekspedisi']= $this->ekspedisi_model->get_data();

                $view = 'templates/backend/biaya_resi_modul/biaya_resi_add';
                show($view, $data);
            } else if(empty($per->is_add)) {
                $this->session->set_flashdata('error', 'Maaf, Anda tidak memiliki hak akses!');
                redirect('biaya_resi', 'refresh');
            } else {
                $this->session->set_flashdata('error', 'Maaf, Anda tidak memiliki hak akses!');
                redirect('biaya_resi', 'refresh');
            }
    ...
    public function biaya_resi_save()
    {
        if($this->session->userdata('logged_in')) {
            ...
            $result=$this->biaya_resi_model->save_data($data);
            if($result!=FALSE) {            
    ...
    public function biaya_resi_edit($id)
    {
        $usr=$this->session->userdata('logged_in');
        $mod=$this->permission_model->get_data_module('biaya_resi');
        $per=$this->permission_model->get_data_akses($mod->module_id,$usr['group']);
        ...
        if($this->session->userdata('logged_in')) {
            if(!empty($per->is_edit)&&($per->is_edit=='true')) {
                ...
                $dt=$this->biaya_resi_model->get_data_edit($id);
                ...
                $data['judul']=$datas->website_title;
                $data['company']=$datas->company_name;
                $data['judul_panel']='Ubah Biaya Resi';

                $view = 'templates/backend/biaya_resi_modul/biaya_resi_edit';
                show($view, $data);
    ...
    public function biaya_resi_update()
    {
        if($this->session->userdata('logged_in')) {
            /* PUT YOUR OWN PROCESS HERE */
            $usr=$this->session->userdata('logged_in');
            ...
            $result=$this->biaya_resi_model->update_data($id_resi,$data);
            if($result!=FALSE) {            
    ...
    public function biaya_resi_delete($id)
    {
        $usr=$this->session->userdata('logged_in');
        $mod=$this->permission_model->get_data_module('biaya_resi');
        $per=$this->permission_model->get_data_akses($mod->module_id,$usr['group']);
        ...
        if($this->session->userdata('logged_in')) {
            if(!empty($per->is_delete)&&($per->is_delete=='true')) {
                /* PUT YOUR OWN PROCESS HERE */
                $result=$this->biaya_resi_model->hapus($id);
                if($result!=FALSE) {            
    ...
    function export() {
        ...
        $data = $this->biaya_resi_model->get_data_export($tgl1,$tgl2);
        ...
        header('Content-Disposition: attachment;filename="biaya_resi_'.date('dMy').'.xls"');
...
/* End of file biaya_resi.php */
/* Location: ./application/controllers/biaya_resi.php */
